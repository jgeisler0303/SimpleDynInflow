...
function [aerodyndata, t]= aerodynAeroField(LAM, TH, FST_file, HubHt, ShearExp, Yaw, dT, Tmax, outputs)
...
if ~exist('ShearExp', 'var') || isempty(ShearExp)
    ShearExp= 0.2;
end
...
driver= SetFASTPar(driver, 'OutFileRoot', '"aerodynAeroField"');
...
driver.Cases.Table(:, 2)= ShearExp;
driver.Cases.Table(:, 3)= SPD(:);
driver.Cases.Table(:, 4)= TTH(:);
driver.Cases.Table(:, 5)= Yaw;
driver.Cases.Table(:, 6)= dT;
driver.Cases.Table(:, 7)= Tmax;
...
[Channels, ChanName, ~, ~] = ReadFASTtext(fullfile(tdir, 'aerodynAeroField.out'));
...
for i= 1:size(driver.Cases.Table, 1)
    ...
    for j= 3:length(ChanName)
        aerodyndata(i_lam, i_th).(ChanName{j})= Channels(i_case, j);
    end
end
